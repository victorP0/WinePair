...
function displayDishResults(paired) {
  fetch(
    `https://api.spoonacular.com/food/wine/dishes?wine=${paired}&apiKey=${key}`
  )
...
function displayWineResults(responseJson, searchTerm) {
  ...
  $(".wine-description").hover(
    function () {
      if ($(this).children(".wineText").html() === "") {
        fetch(
          `https://api.spoonacular.com/food/wine/dishes?wine=${$(this)
            .children(".wine-name")
            .text()}&apiKey=${key}`
        )
          .then((response) => response.json())
          ...
          .catch((error) =>
            console.log("Cuisine went wrong. Error was " + error)
          );
      } else {
  ...
  $("#descriptionText").html(
    wineDescriptionTemplate(
      responseJson.productMatches[0],
...
function watchFoodButton() {
  $("section").on("click", "#food-submit", (event) => {
    ...
    fetch(
      `https://api.spoonacular.com/food/wine/pairing?food=${searchTerm}&apiKey=27b52f3cd6fc4b49a9d3460e3f593f0b`
    )
      .then((response) => response.json())
